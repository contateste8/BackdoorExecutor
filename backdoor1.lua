...
ExecuteButton.MouseButton1Click:Connect(function()
    ...
    notifyY("Success", "Script Executed")
...
function ScanBackdoor()
    ...
    local codeID = randomString(16)
    local finalText = "Scanning... " .. codeID
    local hintScript = [[local h = Instance.new("Hint", workspace); h.Name = "]] .. codeID .. [["; h.Text = "]] .. finalText .. [["]]
        ...
        for _, obj in ipairs(localBase:GetDescendants()) do
            if obj:IsA("RemoteEvent") then
                local success = pcall(function()
                    obj:FireServer(hintScript)
                    enviados += 1
                end)
                task.wait(0.05)
            elseif obj:IsA("RemoteFunction") then
                local success = pcall(function()
                    obj:InvokeServer(hintScript)
                    enviados += 1
                end)
                task.wait(0.05)
            end
    ...
    if enviados == 0 then
        logToConsole("ERROR", "No remotes found", Color3.fromRGB(255, 100, 100))
        notifyY("Error", "No remotes found")
        return
    end
    ...
    task.spawn(function()
        ...
        for _ = 1, 150 do
            for _, obj in ipairs(workspace:GetChildren()) do
                if obj:IsA("Hint") and obj.Name == codeID and obj.Text == finalText then
                    found = obj
                    break
                end
            end
        ...
        if found then
            ...
            logToConsole("SUCCESS", "Backdoor found! " .. string.format("%.2f", duration) .. " ", Color3.fromRGB(100, 255, 100))
            notifyY("Success", "Backdoor found!")
            
            local removerScript = [[for _, h in pairs(workspace:GetChildren()) do if h:IsA("Hint") and h.Name == "]]..codeID..[[" then h:Destroy() end end]]

            for _, localBase in ipairs(lugares) do
                for _, obj in ipairs(localBase:GetDescendants()) do
                    if obj:IsA("RemoteEvent") then
                        pcall(function()
                            obj:FireServer(removerScript)
                        end)
                        task.wait(0.05)
                    elseif obj:IsA("RemoteFunction") then
                        pcall(function()
                            obj:InvokeServer(removerScript)
                        end)
                        task.wait(0.05)
                    end
                end
            ...
            logToConsole("ERROR", "Backdoor not found!", Color3.fromRGB(255, 100, 100))
            notifyY("Error", "Backdoor not found!")
        end
    end)
end
